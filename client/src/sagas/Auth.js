...
function* createUserWithEmailPassword({payload}) {
    ...
    try {
        ...
        if (signUpUser.message) {
            ...
            const signUpUserDB = yield call(createNewUserClientDataBase, name, email, phone);
            localStorage.setItem('user_id', signUpUser.user.uid);
            localStorage.setItem('user_user', JSON.stringify(signUpUserDB.data));
            yield put(userSignUpSuccess({uid:signUpUser.user.uid, user:signUpUserDB.data}));
        }
    } catch (error) {
    ...
    try {
        ...
        if (signUpUser.message) {
            ...
            if (User.data) {
                ...
                yield put(userGoogleSignInSuccess({uid:signUpUser.user.uid, user:User.data}));
            } else {
    ...
    try {
        ...
        if (signUpUser.message) {
            ...
            if (User.data) {
                localStorage.setItem('user_id', signUpUser.user.uid);
                localStorage.setItem('user_user', JSON.stringify(User.data));
                yield put(userFacebookSignInSuccess({uid:signUpUser.user.uid, user:User.data}));
            } else {
                yield call(deletingASignedFirebaseUser);
                yield put(showAuthMessage("You need to Sign Up First"));
            }
    ...
    try {
        ...
        if (signInUser.message) {
            ...
            if (User.data) {
                ...
                localStorage.setItem('user_user', JSON.stringify(User.data));
                yield put(userSignInSuccess({uid:signInUser.user.uid, user:User.data}));
            } else {
                yield call(deletingASignedFirebaseUser);
                yield put(showAuthMessage("You need to Sign Up First"));
            }
